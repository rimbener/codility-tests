...
function solution(A) {
    let sum1 = A[0];
    const part2 = A.slice(1, A.length);
    let sum2 = sum(part2);
    let results = Math.abs(sum1 - sum2);

    for (let index = 0; index < part2.length - 1; index++) {
        sum1 += part2[index];
        sum2 -= part2[index];
        dif = Math.abs(sum1 - sum2);
        results = Math.min(dif, results);
    }
    return results;
}
...
function createTestArray(length, from, to) {
    let arr = [];
    for (let i = 1; i < length + 1; i++) {
        arr.push(getRndInteger(from, to));
    }
    return arr;
}
